...
By default (skip_pair_validation=True), pair validation is disabled for
tiktoken compatibility. This makes the encoder effectively a greedy
longest-match algorithm, which is what tiktoken uses.
...
struct BacktrackEncoder(Movable):
    ...
    """If True, skip is_valid_token_pair checks (for tiktoken compatibility)."""
    ...
    fn __init__(
        ...
        Args:
            ...
            skip_pair_validation: If True, skip is_valid_token_pair checks.
                Default True for tiktoken compatibility.
        """
        self._vocab = vocab^
        self._trie = trie^
        self._text = text.copy()
        self._tokens = List[Int](capacity=len(text) // 3)
        self._pos = 0
        ...
        self._bitfield = BitField(len(text) + 1)
    ...
    fn __moveinit__(out self, deinit existing: Self):
        """Move constructor."""
        self._vocab = existing._vocab^
        self._trie = existing._trie^
        self._text = existing._text^
        self._tokens = existing._tokens^
        self._next_token = existing._next_token
        self._next_token_len = existing._next_token_len
        self._pos = existing._pos
        self._bitfield = existing._bitfield^
        self._skip_pair_validation = existing._skip_pair_validation

    fn _next_match(self, pos: Int) -> TrieLookupResult:
        """
    ...
    fn _get_token_len(self, token_id: Int) -> Int:
        """
    ...
    fn get_tokens(self) -> List[Int]:
        """Get the accumulated tokens (copy)."""
        return self._tokens.copy()

    fn into_tokens(deinit self) -> List[Int]:
