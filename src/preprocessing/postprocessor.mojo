"""
Explicit token-sequence post-processing stages.
...
struct EncodingOutput(Copyable, Movable):
    ...
    def add_token(mut self, id: Int, type_id: Int = 0, is_special: Bool = False):
...
trait PostProcessor:
    ...
    def process(self, encoding: EncodingOutput, add_special_tokens: Bool) -> EncodingOutput:
...
struct BertPostProcessor(PostProcessor):
    ...
    def __init__(out self, cls_id: Int = 101, sep_id: Int = 102):
...
struct TemplatePostProcessor(PostProcessor):
    ...
    def __init__(out self, single: String, pair: String):
    ...
    def _apply_template(
        ...
        var result = EncodingOutput()
        var i = 0
        var n = template.byte_length()

        while i < n:
            var ch = String(template[byte=i])
            if ch == "$" and i + 1 < n:
                var next_ch = String(template[byte=i + 1])
                if next_ch == "A":
                    # Insert sequence A
                ...
                var token_start = i
                while i < n and String(template[byte=i]) != "]":
                    i += 1
                if i < n:
                    i += 1  # Skip ]
                var token_text = String(template[byte=token_start:i])
...
struct LlamaPostProcessor(PostProcessor):
    ...
    def __init__(out self, bos_id: Int = 1, eos_id: Int = 2):
