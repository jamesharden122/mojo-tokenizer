"""
SIMD-optimized whitespace and boundary operations.
...
def trim_whitespace(data: String) -> String:
    ...
    var end = data.byte_length()
    while end > start:
        var c = UInt8(ord(data[byte=end - 1]))
        if not is_whitespace(c):
            break
        end -= 1

    return String(data[byte=start:end])
...
def is_boundary_byte(code: UInt8) -> Bool:
    ...
    return (
        code == 32
        or (code >= 33 and code <= 47)  # space
        or (code >= 58 and code <= 64)  # !"#$%&'()*+,-./
        or (code >= 91 and code <= 96)  # :;<=>?@
        or (code >= 123 and code <= 126)  # [\]^_`
    )  # {|}~
...
def find_boundaries_simd(data: String, start: Int = 0) -> List[Int]:
    ...
    while pos + SIMD_WIDTH <= n:
        ...
        comptime for i in range(SIMD_WIDTH):
            chunk[i] = ptr[unsafe_offset=pos + i]
    ...
    # Scalar tail for remaining bytes
    while pos < n:
        if is_boundary_byte(ptr[unsafe_offset=pos]):
