...
struct PreToken(Copyable, Movable):
    ...
    fn __init__(out self, text: String, start: Int, end: Int):
        """Create a pre-token."""
        self.text = text
        self.start = start
        self.end = end
...
struct WhitespacePreTokenizer(PreTokenizer):
    ...
    fn pre_tokenize(self, text: String) -> List[PreToken]:
        ...
        while start < n:
            ...
            var end = start
            while end < n:
                var code = ord(text[end])
                if is_whitespace(UInt8(code)):
                    break
                end += 1

            if end > start:
                result.append(PreToken(text[start:end], start, end))
...
struct ByteLevelPreTokenizer(PreTokenizer):
    ...
    fn __init__(out self, add_prefix_space: Bool = True):
    ...
    fn pre_tokenize(self, text: String) -> List[PreToken]:
        ...
        while pos < n:
            ...
            var word_end = pos
            while word_end < n and ord(text[word_end]) != 32:
                word_end += 1

            if word_end > word_start:
                var word_text = text[word_start:word_end]
...
struct PunctuationPreTokenizer(PreTokenizer):
    ...
    fn __init__(out self, behavior: String = "isolated"):
...
struct DigitPreTokenizer(PreTokenizer):
    ...
    fn __init__(out self, individual_digits: Bool = False):
...
struct SplitPreTokenizer(PreTokenizer):
    ...
    fn __init__(out self, pattern: String, behavior: String = "removed"):
        ...
        while i <= len(text) - len(self.pattern):
            ...
            if is_match:
                ...
                if self.behavior == "isolated":
                    result.append(PreToken(self.pattern, i, i + len(self.pattern)))
                # "removed" behavior: don't add the pattern

                current_start = i + len(self.pattern)
                i = current_start
            else:
                i += 1
