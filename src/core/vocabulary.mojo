"""
Core vocabulary storage for BPE tokenizers.
...
struct Vocabulary(Copyable, Movable):
    ...
    def add_token_bytes(mut self, token: String, id: Int, raw_bytes: List[UInt8]):
        ...
        This supports vocabularies whose high bytes (>127) must be stored
        exactly rather than reconstructed from UTF-8 token text.
    ...
    def build_backtrack_tables(mut self):
        ...
        for token_id in range(n):
            # Use exact raw bytes when available, otherwise fall back to text.
